...
function AdvisorLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try { 
      const response = await fetch('http://localhost:8000/api/advisors/login', {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        localStorage.setItem("advisor", JSON.stringify(data.data.advisor));
        localStorage.setItem("advisorId", data.data.advisor._id);
      ...
      <div className="w-full max-w-4xl grid lg:grid-cols-2 gap-8 lg:gap-16">
        ...
        <div className="w-full">
          <div className="p-8 space-y-8 bg-white rounded-3xl shadow-2xl">
            <h2 className="text-3xl font-bold text-gray-900 text-center">Advisor Login</h2>
